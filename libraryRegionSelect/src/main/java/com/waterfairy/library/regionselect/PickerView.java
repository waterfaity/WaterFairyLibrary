 ...
 */
public class PickerView<T extends PickerView.DataInt> extends View {
    ...
    public T getSelectData() {
        if (mDataList == null || mDataList.size() <= mCurrentSelected) return null;
     ...
     * 选择选中的内容
     */
    public void setSelected(DataInt mSelectItem) {
        for (int i = 0; i < mDataList.size(); i++) {
            if (mDataList.get(i) == mSelectItem) {
                setSelected(i);
                break;
            }
        }
...
//        mMinTextSize = mMaxTextSize / 2.2f;
     ...
     * @param minAlpha
     */
    public void setTextAlpha(int maxAlpha, int minAlpha) {
        mMaxTextAlpha = maxAlpha;
        mMinTextAlpha = minAlpha;
        invalidate();
    }
